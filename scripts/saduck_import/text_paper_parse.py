...
OUT = sys.argv[3] if len(sys.argv) > 3 else _p("WorkBuddy", "2026-09-19-20-27-33", ".tmp", "gongkao", "text_parsed.json")
TOTAL = int(sys.argv[4]) if len(sys.argv) > 4 else 135
...
def main():
    ...
    for ri, r in enumerate(lines[:ans_start]):
        t = r["t"]
        if SECTION.match(t) or t.startswith("根据题目要求"):
            continue
        m = QNO.match(t)
        mo = OPT.match(t)
    ...
    for n in range(1, TOTAL + 1):
        ...
        result[n] = {"id": n, "stem": q.get("stem", ""), "options": q.get("options", {}),
                     "answer": answer, "explain": ex, "section": q.get("section", ""), "part": part_from(q.get("section", ""))}
    ...
    for n in [x for x in (1, 2, 61, TOTAL) if x in result]:
        r = result[n]
